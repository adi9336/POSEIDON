...
class QueryResponse(BaseModel):
    ...
    final_answer: Optional[str] = Field(
        None,
        description="The final processed answer from the workflow, if available"
    )
...
async def process_query(request: QueryRequest):
    ...
    try:
        ...
        logger.info("="*60)
        logger.info(f"✅ QUERY COMPLETED: {len(processed.get('data', []))} rows returned")
        logger.info("="*60)
