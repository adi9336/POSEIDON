...
def _fetch_with_params(
    ...
    try:
        ...
        if (
            ...
            and hasattr(intent, "lon")
            and intent.lon is not None
        ):
            lat = intent.lat
            lon = intent.lon
        ...
        # If not, try to resolve from location name
            ...
            print(
                f"   🗺️  No coordinates provided. Resolving location: '{intent.location}'"
            )
        ...
        print(
            f"   📐 Range: lat [{lat_min:.1f}, {lat_max:.1f}], lon [{lon_min:.1f}, {lon_max:.1f}]"
        )
        ...
        if not hasattr(intent, "time_range") or not intent.time_range:
            ...
            if end_date > today:
                print(
                    f"   ⚠️  End date {end_date} is in the future. Adjusting to today ({today})."
                )
                end_date = today
...
if __name__ == "__main__":
    ...
    if not df2.empty:
        ...
        if "temp" in df2.columns:
            print(
                f"Temperature range: {df2['temp'].min():.2f}°C to {df2['temp'].max():.2f}°C"
            )
    else:
        print("\n❌ No data found")
